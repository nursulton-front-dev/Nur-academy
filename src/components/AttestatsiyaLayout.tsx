...
import { mockModules, resetAllProgress } from '../data/attestatsiyaMocks';
...
export default function AttestatsiyaLayout() {
  ...
  const handleReset = () => {
    if (confirm("Haqiqatdan ham barcha o'zlashtirilgan darslar va test natijalarini tozalamoqchimisiz?")) {
      resetAllProgress();
    }
  };
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-primary-bg border-r border-border-card text-text-primary transition-colors duration-250">
      {/* Sidebar Header */}
      <div className="p-4 border-b border-border-card space-y-2">
        ...
        <button
          onClick={handleReset}
          className="w-full flex items-center gap-1.5 px-2 py-1 text-[11px] font-semibold text-rose-500 hover:text-rose-600 hover:bg-rose-500/5 rounded-md border border-rose-500/20 transition-all text-left mt-1 cursor-pointer"
        >
                  ...
                  {isExpanded && (
                    <div className="pl-4 pr-1 py-1 space-y-1 border-l border-border-card ml-4">
                      {mod.lessons.map((les) => {
                        ...
                        if (isCompleted) {
                          statusIcon = <CheckCircle2 className="w-4 h-4 text-[#10B981]" />;
                        } else if (isLocked) {
        ...
        <div>
          ...
          <div className="space-y-1">
            ...
            <Link
              ...
              onClick={() => setMobileMenuOpen(false)}
              className={`flex items-center space-x-2.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                location.pathname === '/attestatsiya/mock-imtihonlar' 
                  ? 'bg-[#3B7DD8]/10 text-[#3B7DD8]' 
                  : 'hover:bg-primary-bg/50 text-text-primary'
              }`}
            >
