...
export default function AttestatsiyaLanding() {
  ...
  return (
    <div className="space-y-8 dark:text-[#EAF3F0] transition-colors duration-250">
      {/* Header Info */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 pb-6 border-b border-[#E3EBE9] dark:border-[#2A3A38]">
        <div className="space-y-2 max-w-2xl">
          ...
          <h1 className="text-3xl sm:text-4xl font-serif font-extrabold text-[#1A2E2E] dark:text-[#EAF3F0]">
            Informatika o'qituvchilari attestatsiyasi
          </h1>
          <p className="text-[#5C7370] dark:text-gray-300 text-sm sm:text-base leading-relaxed">
      ...
      {/* Progress Card */}
      <div className="bg-[#F6F9F8] dark:bg-[#16201F] p-6 rounded-xl border border-[#E3EBE9] dark:border-[#2A3A38]">
        <div className="flex justify-between items-center mb-3">
          <div className="flex items-center space-x-2">
            <Clock className="w-5 h-5 text-[#3B7DD8]" />
            <span className="font-semibold text-sm text-[#1A2E2E] dark:text-[#EAF3F0]">Umumiy o'zlashtirish progressi</span>
          </div>
          <span className="font-bold text-sm text-[#3B7DD8]">{progressPercent}%</span>
        </div>
        <div className="w-full bg-[#E3EBE9] dark:bg-[#2A3A38] h-3.5 rounded-full overflow-hidden">
          <div 
            className="bg-[#3B7DD8] h-full rounded-full transition-all duration-500" 
            style={{ width: `${progressPercent}%` }}
          ></div>
        </div>
        <div className="flex justify-between items-center mt-3 text-xs text-[#5C7370] dark:text-gray-400">
      ...
      <div className="space-y-4">
        <h2 className="text-xl sm:text-2xl font-serif font-bold text-[#1A2E2E] dark:text-[#EAF3F0]">
          O'quv modullari
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {mockModules.map((mod) => {
            ...
            let statusBadge = (
              <span className="inline-flex items-center space-x-1 text-xs bg-gray-100 dark:bg-gray-800 text-gray-500 px-2 py-0.5 rounded-md font-medium">
              ...
              <div 
                ...
                className={`p-6 rounded-xl border transition-all duration-200 flex flex-col justify-between ${
                  isLocked 
                    ...
                    : 'bg-white dark:bg-[#1E2B29] border-[#E3EBE9] dark:border-[#2A3A38] hover:border-[#3B7DD8] hover:shadow-md'
                }`}
              >
                <div>
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="font-serif font-bold text-lg text-[#1A2E2E] dark:text-[#EAF3F0] line-clamp-1">
                      {mod.title}
                    </h3>
                    {statusBadge}
                  </div>
                  <p className="text-[#5C7370] dark:text-gray-300 text-xs sm:text-sm leading-relaxed mb-4 line-clamp-2">
                    {mod.description}
                  </p>
                </div>

                <div className="flex justify-between items-center text-xs text-[#5C7370] dark:text-gray-400 pt-2 border-t border-[#E3EBE9]/50 dark:border-[#2A3A38]/50">
